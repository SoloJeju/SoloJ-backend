...
public class ReportController {
    ...
    public ApiResponse<ReportResponseDto> createReport(
        ...
        ReportResponseDto response = reportService.createReport(user, dto);
    ...
    public ApiResponse<ReportHistoryResponseDto> getMyReports(
        ...
        ReportHistoryResponseDto response = reportService.getMyReports(user, pageable, status);
        return ApiResponse.onSuccess(response);
    }
    ...
    public ApiResponse<ReportDetailDto> getReportDetail(
        ...
        ReportDetailDto detail = reportService.getReportDetail(user, reportId);
    ...
    public ApiResponse<String> cancelReport(
            ...
            @Parameter(description = "신고 ID", required = true)
            @PathVariable Long reportId) {
        reportService.cancelReport(user, reportId);
        return ApiResponse.onSuccess("신고가 취소되었습니다.");
    }

    @GetMapping("/statistics")
    @Operation(summary = "사용자 신고 통계", description = "로그인한 사용자의 신고 통계를 조회합니다.")
    public ApiResponse<UserReportStatsDto> getReportStatistics(@AuthUser User user) {
        UserReportStatsDto stats = reportService.getUserReportStatistics(user);
    ...
    public ApiResponse<BulkReportResponseDto> bulkReport(
        ...
        BulkReportResponseDto response = reportService.bulkReport(user, dto);
        return ApiResponse.onSuccess(response);
    }
    ...
    public ApiResponse<Map<String, Boolean>> checkDuplicate(
        ...
        boolean isDuplicate = reportService.checkDuplicateReport(user, targetUserId, targetPostId, targetCommentId);
    ...
    public ApiResponse<List<ReportNotificationDto>> getReportNotifications(
        ...
        List<ReportNotificationDto> notifications = reportService.getReportNotifications(user, unreadOnly);
    ...
    public ApiResponse<String> markNotificationAsRead(
        ...
        reportService.markNotificationAsRead(user, notificationId);
